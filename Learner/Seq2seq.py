...
class GRU:
    def __init__(self, is_training, word_embeddings, settings):
        ...
        #Bidirectional GRU
        gru_cell_forward = tf.contrib.rnn.GRUCell(num_units = gru_size)
        gru_cell_backward = tf.contrib.rnn.GRUCell(num_units = gru_size)
        ...
        cell_forward = tf.contrib.rnn.MultiRNNCell([gru_cell_backward] * settings.num_layers)
        ...
        with tf.variable_scope('GRU_FORWARD') as scope:
            for step in range(num_steps):
                if step > 0:
                    scope.reuse_variables()
                (cell_output_backward, state_backward) = cell_backward(inputs_forward[:, step, :], state_backward)
                outputs_backward.append(cell_output_forward)
